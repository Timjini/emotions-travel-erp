                ...
                <div class="px-6 py-4 space-y-4">
                    ...
                    <div>
                        ...
                        <x-text-input id="number_of_people" name="number_of_people" type="text" class="mt-1 block w-full" 
                            :value="old('number_of_people', $file->number_of_people)" required />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="start_date" name="start_date" type="date" class="mt-1 block w-full" 
                                :value="old('start_date', $file->start_date->format('Y-m-d'))" required />
                            <x-input-error class="mt-2" :messages="$errors->get('start_date')" />
                        </div>
                        <div>
                            <x-input-label for="end_date" :value="__('End Date')" />
                            <x-text-input id="end_date" name="end_date" type="date" class="mt-1 block w-full" 
                                :value="old('end_date', $file->end_date->format('Y-m-d'))" required />
                        ...
                        <div>
                            ...
                            <select id="program_id" name="program_id" class="rounded bg-gray-50 border text-gray-900  flex-1 min-w-0 text-sm border-gray-300 p-2.5 mt-1 block w-full">
                                ...
                                @foreach($programs as $program)
                                    <option value="{{ $program->id }}" {{ old('program_id', $file->program_id) == $program->id ? 'selected' : '' }}>
                                        {{ $program->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="destination_id" name="destination_id" class="rounded bg-gray-50 border text-gray-900  flex-1 min-w-0 text-sm border-gray-300 p-2.5 mt-1 block w-full">
                                ...
                                @foreach($destinations as $destination)
                                    <option value="{{ $destination->id }}" {{ old('destination_id', $file->destination_id) == $destination->id ? 'selected' : '' }}>
                                        {{ $destination->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="currency_id" name="currency_id" class="rounded bg-gray-50 border text-gray-900  flex-1 min-w-0 text-sm border-gray-300 p-2.5 mt-1 block w-full">
                                ...
                                @foreach($currencies as $currency)
                                    <option value="{{ $currency->id }}" {{ old('currency_id', $file->currency_id) == $currency->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <textarea id="note" name="note" rows="3" class="rounded bg-gray-50 border text-gray-900  flex-1 min-w-0 text-sm border-gray-300 p-2.5 mt-1 block w-full">{{ old('note', $file->note) }}</textarea>
                        <x-input-error class="mt-2" :messages="$errors->get('note')" />
                    </div>
                </div>
                
                <div class="px-6 py-4 border-t border-gray-200 flex justify-end">
                    <!-- <x-secondary-button class="mr-2" onclick="window.location='{{ route('files.show', $file) }}'">
                        Cancel
                    </x-secondary-button> -->

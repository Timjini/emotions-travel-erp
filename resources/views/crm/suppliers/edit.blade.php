    ...
    <div class="py-12">
        <div class="max-w-4xl sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('suppliers.update', $supplier) }}" x-data="{ loading: false }" @submit="loading = true">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-6">
                        ...
                        <select id="status" name="status" class="mt-2 rounded bg-gray-50 border text-gray-900  block flex-1 min-w-0 w-full text-sm border-gray-300 p-2.5 mt-1 block w-full" required>
                            @foreach(\App\Enums\CustomerStatus::cases() as $status)
                                <option value="{{ $status->value }}" {{ old('status', $supplier->status) == $status->value ? 'selected' : '' }}>
                                    {{ $status->label() }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="invoicing_entity" name="invoicing_entity" type="text" class="mt-1 block w-full"
                                value="{{ old('invoicing_entity', $supplier->invoicing_entity) }}" required />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="post_code" name="post_code" type="text" class="mt-1 block w-full"
                                value="{{ old('post_code', $supplier->post_code) }}" required />
                        ...
                        <div>
                            ...
                            <select id="country" name="country" class="mt-2 rounded bg-gray-50 border text-gray-900  block flex-1 min-w-0 w-full text-sm border-gray-300 p-2.5 mt-1 block w-full" required>
                                ...
                                @foreach(config('countries') as $code => $name)
                                    <option value="{{ $code }}" {{ old('country', $supplier->country) == $code ? 'selected' : '' }}>
                                        {{ $name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="mt-1 flex rounded-md shadow-sm">
                            ...
                            <x-text-input id="website" name="website" type="text" class="block w-full min-w-0 flex-1 rounded-none rounded-r-md"
                                value="{{ old('website', str_replace(['https://', 'http://'], '', $supplier->website ?? '')) }}" 
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="phone_1" name="phone_1" type="tel" class="mt-1 block w-full"
                                value="{{ old('phone_1', $supplier->phone_1) }}" />
                        </div>
                        ...
                        <div>
                            ...
                            <x-text-input id="phone_2" name="phone_2" type="tel" class="mt-1 block w-full"
                                value="{{ old('phone_2', $supplier->phone_2) }}" />
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <select id="type" name="type" class="mt-2 rounded bg-gray-50 border text-gray-900  block flex-1 min-w-0 w-full text-sm border-gray-300 p-2.5 mt-1 block w-full" required>
                                @foreach(\App\Enums\CustomerType::cases() as $type)
                                    <option value="{{ $type->value }}" {{ old('type', $supplier->type->value) == $type->value ? 'selected' : '' }}>
                                        {{ $type->label() }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="iban" name="iban" type="text" class="mt-1 block w-full font-mono"
                                value="{{ old('iban', $supplier->iban) }}" placeholder="XX00 XXXX 0000 0000 0000 0000" />
                        </div>
                        ...
                        <div>
                            ...
                            <x-text-input id="swift_code" name="swift_code" type="text" class="mt-1 block w-full font-mono"
                                value="{{ old('swift_code', $supplier->swift_code) }}" placeholder="XXXXXX00XXX" />
                        </div>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <textarea id="notes" name="notes" rows="3" class="mt-2 rounded bg-gray-50 border text-gray-900  block flex-1 min-w-0 w-full text-sm border-gray-300 p-2.5 mt-1 block w-full">{{ old('notes', $supplier->notes) }}</textarea>
                    </div>

                    <div class="flex justify-end space-x-4 pt-6 border-t border-gray-200">
                        <x-secondary-link  href="'{{ route('suppliers.show', $supplier) }}'">
                            {{ __('Cancel') }}
                        </x-secondary-link>
                        <x-loading-button label="{{ __('Update Supplier') }}" />
                    </div>
                </form>
            </div>
        </div>
